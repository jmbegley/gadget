...
this site (select "Clone or download"). As a guide, the
...
The recruitment $<$functionname$>$ defines which recruitment function to use to calculate the number of recruits to be added to the spawned stock.  Currently there are 6 recruitment functions defined, and the valid recruitment function names are:
...
baleen - use the Baleen recruitment function\newline
hockeystick - use the HockeyStick recruitment function
...
\subsection{Baleen Recruitment Function}
This recruitment function calculates the number of recruits to be added to the spawned stock, as a function of the spawning stock biomass (see equation~\ref{eq:ssb}), based on the Baleen recruitment relationship given by the following equation:

\begin{equation}\label{eq:rec5}
R = p_{0} S (1.0 + p_{1} (1.0 - {{S / p_{2}} ^ {p_{3}}}))
\end{equation}

Hence, to specify this recruitment function, the file format required is:

{\small\begin{verbatim}
recruitment            baleen  <vector of 4 parameters>
\end{verbatim}}

\subsection{HockeyStick Recruitment Function}
This recruitment function calculates the number of recruits to be added to the spawned stock, as a function of the spawning stock biomass (see equation~\ref{eq:ssb}), based on the HockeyStick recruitment relationship given by the following equation:

\begin{equation}\label{eq:rec6}
R = 
\begin{cases}
p_{0} & \textrm{if $S > p_{1}$} \\
p_0 S / p_{1} & \textrm{if $S \leq p_{1}$}
\end{cases}
\end{equation}
...
{\small\begin{verbatim}
recruitment            hockeystick  <vector of 2 parameters>
\end{verbatim}}
...
overconsumption      <0 or 1> ; 1 to take overconsumption into account
areaaggfile          <area aggregation file specifying areas>
lenaggfile           <length aggregation file specifying lengths> ; optional
ageaggfile           <age aggregation file specifying ages>
fleetnames           <vector of the names of the fleets>
stocknames           <vector of the names of the stocks>
\end{verbatim}}
...
\bigskip
It is only necessary to specify one of the $<$length aggregation file$>$ and $<$age aggregation file$>$ entries. Most of the CatchStatistcs likelihood functions are based on mean length or weight at age, so require the $<$age aggregation file$>$ entry, but there are two functions that are based on the mean weight at length, so these functions require the $<$length aggregation file$>$ entry instead.

\bigskip
The $<$fleetnames$>$ vector contains a list of all the fleets to be aggregated into a single pseudo fleet for the purposes of the data comparison.  Similarly, the $<$stocknames$>$ vector contains a list of all the stocks to be aggregated into a single pseudo stock.

\bigskip
The $<$function name$>$ defines what likelihood function is to be used to compare the modelled statistical data to the input statistical data.  Currently, there are 7 likelihood functions defined, and the format of the statistical data given in the file specified by $<$datafile$>$ depends on the likelihood function used.  The valid functions are:
...
\subsection{Weighted Sum of Squares of Mean Weight at Length\newline With Given Standard Deviation}
This likelihood function calculates the likelihood score based on a weighted sum of squares of the mean weight at length, with the weighting given the variance of weight of the input population, as shown in equation~\ref{eq:catchstat6} below:

\begin{equation}\label{eq:catchstat6}
\ell = \sum_{\it time}\sum_{\it areas}\sum_{\it len} \Big(\frac{(x_{trl}-\mu_{trl})^2} {s_{trl}^2} N_{trl}\Big)
\end{equation}

where:\newline
$<$ x $>$ is the sample mean weight at length from the data\newline
$<\mu>$ is the mean weight at length calculated from the model\newline
$<$ s $>$ is the standard deviation of the weight at length from the data\newline
$<$ N $>$ is the sample size
...
{\small\begin{verbatim}
year>  <step>  <area>  <length>  <number>  <mean>  <stddev>
\end{verbatim}}

where $<$number$>$ is the number of samples for the timestep/area/length combination, $<$mean$>$ is the mean weight at length of these samples and $<$stddev$>$ is the standard deviation of the weight at length of these samples. Note that for this CatchStatistics function, the length aggregation file instead of the age aggregation file must be specified.

\subsection{Unweighted Sum of Squares of Mean Length}
This likelihood function calculates the likelihood score based on a unweighted sum of squares of the mean weight at length, with the variance of the weight at length of the population assumed to be 1, as shown in equation~\ref{eq:catchstat7} below:

\begin{equation}\label{eq:catchstat7}
\ell = \sum_{\it time}\sum_{\it areas}\sum_{\it len} \Big((x_{trl}-\mu_{trl})^2 N_{trl}\Big)
\end{equation}
...
where $<$number$>$ is the number of samples for the timestep/area/length combination, and $<$mean$>$ is the mean weigth at length of these samples. Note that for this CatchStatistics function, the length aggregation file instead of the age aggregation file must be specified.
